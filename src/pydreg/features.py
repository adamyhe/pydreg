...
def _binned_sums_batch_numba(csum, offsets, window_sizes, half_n_windows, n_features):
    ...
    in isolation, and -- since this is the majority (60-80%+) of
    per-cluster feature-extraction time at realistic informative-position
    densities, not just of this one function -- a real win specifically for
    the `cupy` backend, where predict is fast enough that extraction is
    already the dominant, unhidden cost for at least the gap-filled-
    positions step on real hardware (TITAN Xp *and* A100, not just top-end
    cards -- see docs/OPTIMIZATION.md's "Real measurements" table)."""
    n = offsets.shape[0]
    ...
    for i in range(n):
        ...
        for z in range(n_zooms):
            ...
            alpha = _ALPHA_LN99 / scale_max
            for h in range(2 * H):
                out[i, col + h] = 1.0 / (1.0 + np.exp(-alpha * (zoom_bins[h] - scale_max)))
            col += 2 * H
    return out
...
def _binned_sums_batch(csum, offsets, window_sizes, half_n_windows):
    ...
    return _binned_sums_batch_numba(csum, offsets, window_sizes, half_n_windows, n_features)
