...
An experimental `cupy` backend (`--backend cupy`) reimplements the same scoring math directly on a CuPy device array instead of going through cuML's SVM code, and should work on older GPUs (compute capability ≥3.0) that `cuml` can't — but it hasn't been validated on real GPU hardware yet, isn't auto-selected, and lives on its own branch pending that validation. See `docs/OPTIMIZATION.md` for details.
...
| ------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--backend {auto,cuml,cupy,sklearn,numpy}` | `auto`      | Scoring backend. `auto` uses cuML when CuPy sees a CUDA device with compute capability ≥7.0, otherwise pure NumPy. `cupy` is an experimental, not-yet-hardware-validated GPU tier that works on older GPUs too (never auto-selected). scikit-learn is selectable explicitly but is not auto-selected (see `docs/OPTIMIZATION.md` for why). An explicit choice raises if that backend isn't actually usable, rather than silently falling back. |
