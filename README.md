...
**GPU requirement:** compute capability ≥3.0 (essentially any CUDA-capable NVIDIA GPU from the last decade-plus, including older Pascal-class cards). Scoring runs pydreg's own RBF kernel implementation directly on a CuPy device array (`pydreg.backend._build_cupy_predict_fn`) rather than through a third-party GPU SVM library, which is also why it isn't limited the way one previously was — see `docs/OPTIMIZATION.md` for the full writeup, including why that library was dropped (real hardware confirmed it silently returned wrong scores below compute capability 7.0).
