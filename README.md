...
An experimental `cupy` backend (`--backend cupy`) reimplements the same scoring math directly on a CuPy device array instead of going through cuML's SVM code, and works on older GPUs (compute capability ≥3.0) that `cuml` can't — confirmed correct (no smoke-test divergence) on real hardware, but still being tuned for speed (kernel fusion + tunable batch sizes via `--cupy-sv-chunk`; see `docs/OPTIMIZATION.md`). Not auto-selected, and lives on its own branch pending further tuning.
...
| ------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `--backend {auto,cuml,cupy,sklearn,numpy}` | `auto`      | Scoring backend. `auto` uses cuML when CuPy sees a CUDA device with compute capability ≥7.0, otherwise pure NumPy. `cupy` is an experimental GPU tier that works on older GPUs too, confirmed correct on real hardware but still being tuned for speed (never auto-selected). scikit-learn is selectable explicitly but is not auto-selected (see `docs/OPTIMIZATION.md` for why). An explicit choice raises if that backend isn't actually usable, rather than silently falling back. |
...
| `-c`, `--cuml-query-chunk N`          | `1048576`        | Positions scored per batch for the cuML backend specifically when `--query-chunk` is not set; ignored by every other backend, including `cupy` (which uses its own `DEFAULT_QUERY_CHUNK` entry).                                                                                              |
